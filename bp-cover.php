...
if ( !defined( 'BPCO_PLUGIN_VERSION' ) )
	define( 'BPCO_PLUGIN_VERSION', '2.1.4' );
		...
		if( $max_upload_size > $file){
    		if($success){
        		...
        		if ( $size[0] > 150 ) {
					...
					$original_file = wp_get_image_editor( $fileurl );
					...
					if( is_wp_error( $original_file ) ){
					    ...
					    $resize = $original_file->resize(150, 150, true);
						$saved = $original_file->save();
						$resized_file = $saved['path'];
					}
					//delete old avatar if it exists
					bp_core_delete_existing_avatar( array( 'item_id' => $user_id, 'avatar_path' => bp_core_avatar_upload_path() .'/avatars/' . $user_id ) );

					$avatar_to_crop = str_replace( bp_core_avatar_upload_path(), '', $resized_file );
					...
					update_user_meta(bp_loggedin_user_id(),'profile_avatar', $fileurl);
